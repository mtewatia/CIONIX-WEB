...
const MediaBlog = () => {
  const { data: posts = [], isLoading } = useBlogPosts("media");
      ...
      <section className="section-padding">
        <div className="container-custom">
          {isLoading ? (
            ...
            <>
              {featured && (
                <Link to={`/media/blog/${featured.slug}`} className="card-ryse overflow-hidden block mb-12 group">
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                {rest.map((post) => (
                  <Link key={post.id} to={`/media/blog/${post.slug}`} className="card-ryse overflow-hidden group">

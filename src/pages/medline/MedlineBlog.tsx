...
const MedlineBlog = () => {
  const { data: posts = [], isLoading } = useBlogPosts("medline");
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="pt-32 pb-16 bg-gradient-to-br from-emerald-900 via-teal-900 to-cyan-900 text-white">
        <div className="container-custom text-center">
          <h1 className="text-4xl md:text-5xl font-bold font-heading mb-4">Medline Blog</h1>
          <p className="text-white/70 text-lg max-w-2xl mx-auto">Healthcare billing insights, compliance updates, and revenue cycle best practices.</p>
        </div>
      </section>
      ...
      <section className="section-padding">
        <div className="container-custom">
          {isLoading ? (
            ...
            <>
              {featured && (
                <Link to={`/medline/blog/${featured.slug}`} className="card-ryse overflow-hidden block mb-12 group">
                  <div className="grid lg:grid-cols-2">
                    ...
                    <div className="p-8 lg:p-12 flex flex-col justify-center">
                      <span className="text-emerald-600 text-sm font-semibold mb-3">{featured.category} • Featured</span>
                      <h2 className="text-2xl lg:text-3xl font-bold font-heading mb-4 group-hover:text-emerald-600 transition-colors">{featured.title}</h2>
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                {rest.map((post) => (
                  <Link key={post.id} to={`/medline/blog/${post.slug}`} className="card-ryse overflow-hidden group">
                    ...
                    <div className="p-6">
                      <span className="text-emerald-600 text-sm font-semibold">{post.category}</span>
                      <h3 className="text-lg font-bold font-heading mt-2 mb-3 group-hover:text-emerald-600 transition-colors">{post.title}</h3>
